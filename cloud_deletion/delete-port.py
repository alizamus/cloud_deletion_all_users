...
def main(argv):
	...
	for tenant in tenants_id:
		...
		neutron_list = neutron.list_ports()
		if neutron_list['ports']:
			for i in range(len(neutron_list['ports'])):
				if neutron_list['ports'][i]['tenant_id']== tenant:
					neutron.delete_port(neutron_list['ports'][i]['id'])
